...
实现由 `ClearLocalDataService` 统一编排；页面只通过 Provider 调用该服务。食品图片只允许位于应用支持目录下的 `food_images` 托管子目录，清除时不得递归删除应用支持根目录或数据库中记录的任意外部路径。日志由 `SafeLogger` 输出内部事件名和异常类型，生产环境完全关闭，不记录异常消息与堆栈。
